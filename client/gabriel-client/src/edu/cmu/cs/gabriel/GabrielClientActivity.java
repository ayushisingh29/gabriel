...
public class GabrielClientActivity extends Activity implements
    ...
    /**
     * Does initialization for the entire application. Called only once even for
    ...
    private void initOnce() {
        ...
        System.out.println("Checked camera and set callback");
    ...
    private void initPerRun(String serverIP, int tokenSize, File latencyFile) {
        ...
        resultThread = new ResultReceivingThread(serverIP,
                Const.RESULT_RECEIVING_PORT, returnMsgHandler);
        resultThread.start();

        videoStreamingThread = new VideoStreamingThread(serverIP,
                Const.VIDEO_STREAM_PORT, returnMsgHandler, tokenController);
        videoStreamingThread.start();

        audioStreamingThread = new AudioStreamingThread(serverIP,
                Const.AUDIO_STREAM_PORT, returnMsgHandler, tokenController);
        audioStreamingThread.start();

        accStreamingThread = new AccStreamingThread(serverIP,
                Const.ACC_STREAM_PORT, returnMsgHandler, tokenController);
    ...
    /**
     * Runs a set of experiments with different server IPs and token numbers. IP
     * list and token sizes are defined in the Const file.
     */
                ...
                GabrielClientActivity.this.runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if ((ipIndex == Const.SERVER_IP_LIST.length)
                                || (tokenIndex == Const.TOKEN_SIZE_LIST.length)) {
                        ...
                        File latencyFile = new File(Const.EXP_DIR
                                .getAbsolutePath()
                                + File.separator
                                + "latency-"
                                + serverIP
                                ...
                                + tokenSize
                                + ".txt");
                        Log.i(LOG_TAG, "Start new experiment - IP: " + serverIP
                                + "\tToken: " + tokenSize);
    ...
    private PreviewCallback previewCallback = new PreviewCallback() {
        ...
        public void onPreviewFrame(byte[] frame, Camera mCamera) {
            if (isRunning) {
                ...
                byte[] audio = recordStream.toByteArray();
                // AudioStreamPush
                System.out.println(String.format("Pushed size: %d",
                        audio.length));
                if (audioStreamingThread != null) {
                    audioStreamingThread.push(audio, parameters);
                }
    ...
    /**
     * Handles messages passed from streaming threads and result receiving
     * threads.
     */
            ...
            if (msg.what == NetworkProtocol.NETWORK_RET_SPEECH) {
                ...
                if (tts != null) {
                    ...
                    if (splitMSGs.length == 1)
                        tts.speak(splitMSGs[0].toString().trim(),
                                TextToSpeech.QUEUE_FLUSH, map); // the only
                                                                // sentence
                    else {
                        tts.speak(splitMSGs[0].toString().trim(),
                                TextToSpeech.QUEUE_FLUSH, null); // the first
                                                                 // sentence
                        for (int i = 1; i < splitMSGs.length - 1; i++) {
                            tts.playSilence(350, TextToSpeech.QUEUE_ADD, null); // add
                                                                                // pause
                                                                                // for
                                                                                // every
                                                                                // period
                            tts.speak(splitMSGs[i].toString().trim(),
                                    TextToSpeech.QUEUE_ADD, null);
                        }
                        tts.playSilence(350, TextToSpeech.QUEUE_ADD, null);
                        tts.speak(splitMSGs[splitMSGs.length - 1].toString()
                                .trim(), TextToSpeech.QUEUE_ADD, map); // the
                                                                       // last
                                                                       // sentence
                    }
                }
    ...
    private void startRecording() {
        recorder = new AudioRecord(MediaRecorder.AudioSource.MIC,
                ...
                Const.RECORDER_AUDIO_ENCODING, bufferSize);
    ...
    private void writeAudioDataToFile() {
        ...
        if (null != recordStream) {
            while (isRecording) {
                ...
                if (AudioRecord.ERROR_INVALID_OPERATION != read) {
                    try {
                        ...
                        // "Wrote %d bytes of recording", read));
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
            ...
            try {
                ...
                Log.e("Recorder", "Failed to close recordStream");
            }
        }
    }
